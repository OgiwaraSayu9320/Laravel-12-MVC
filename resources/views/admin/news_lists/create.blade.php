...
@section('title', 'Thêm danh mục tin tức cấp 1')

@section('content')
    <div class="container-fluid">
        <form action="{{ route('admin.news-lists.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h1 class="h3 mb-0">Thêm mới <strong>Danh mục cấp 1</strong></h1>
                                ...
                                @foreach ($config_langs as $key => $name)
                                    <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="ten{{ $key }}" class="form-control @error('ten' . $key) is-invalid @enderror" value="{{ old('ten' . $key) }}" placeholder="Danh mục tin tức...">
                                            @error('ten' . $key)

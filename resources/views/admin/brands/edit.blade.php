...
@section('title', 'Cập nhật thương hiệu: ' . $brand->tenvi)
    ...
    <div class="container-fluid">
        <form action="{{ route('admin.brands.update', $brand->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h1 class="h3 mb-0">Cập nhật <strong>Thương hiệu</strong>: <span class="text-primary">{{ $brand->tenvi }}</span></h1>
                                ...
                                @foreach ($config_langs as $key => $name)
                                    <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="ten{{ $key }}" class="form-control @error('ten' . $key) is-invalid @enderror" value="{{ old('ten' . $key, $brand->{"ten$key"}) }}" required>
                                            @error('ten' . $key)
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="tenkhongdau{{ $key }}" class="form-control" value="{{ old('tenkhongdau' . $key, $brand->{"tenkhongdau$key"}) }}">
                                        </div>
                                        ...
                                        <div class="row g-2">
                                            <div class="col-12 mb-2">
                                                ...
                                                <input type="text" name="title{{ $key }}" class="form-control" value="{{ old('title' . $key, $brand->{"title$key"}) }}">
                                            </div>
                                            <div class="col-12 mb-2">
                                                <label class="small fw-bold">Từ khóa SEO</label>
                                                <input type="text" name="keywords{{ $key }}" class="form-control" value="{{ old('keywords' . $key, $brand->{"keywords$key"}) }}">
                                            </div>
                                            <div class="col-12">
                                                <label class="small fw-bold">Mô tả SEO</label>
                                                <textarea name="description{{ $key }}" class="form-control" rows="2">{{ old('description' . $key, $brand->{"description$key"}) }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
                    ...
                    <div class="card shadow-sm mb-4">
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <div id="photo-preview" class="text-center rounded border d-flex align-items-center justify-content-center bg-light" style="min-height: 150px">
                                    @if($brand->photo && Storage::disk('public')->exists($brand->photo))
                                        <img src="{{ asset('storage/' . $brand->photo) }}" class="img-fluid rounded border shadow-sm p-1 bg-white">
                                    @else
                                        <span class="text-muted small">Cập nhật logo</span>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" name="stt" class="form-control" value="{{ old('stt', $brand->stt) }}" min="0">
                            </div>

                            <div class="form-check form-switch mb-2">
                                <input class="form-check-input" type="checkbox" name="hienthi" value="1" id="hienthi" {{ $brand->hienthi ? 'checked' : '' }}>
                                <label class="form-check-label fw-bold" for="hienthi">Hiển thị</label>
                            </div>

                            <div class="form-check form-switch mb-2">
                                <input class="form-check-input" type="checkbox" name="noibat" value="1" id="noibat" {{ $brand->noibat ? 'checked' : '' }}>
